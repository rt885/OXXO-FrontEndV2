...
const SearchBarManagement = ({ onSearchComplete }) => {
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const response = await fetch(
        `https://oxxo-esls-backend.onrender.com/api/products/prices/${tiendaId}`,
        {
          method: "GET",
          headers: {
      ...
      onSearchComplete(data); // Llama al callback con los datos obtenidos
    } catch (error) {
  ...
  return (
    <div className="w-full bg-white lg:h-20 flex lg:justify-between justify-center items-center px-3 lg:px-10 shadow-md">
      <p className="lg:text-base text-sm hidden lg:block">ESL Management</p>
...
export default SearchBarManagement;
